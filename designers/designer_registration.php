...
if (isset($_POST['name'])) {
	...
	if($password != $cpassword){
		echo 'Your passwords do not match., <a href="designer_registration.php">Refill here</a>';
		exit();
	}
	$sql = mysql_query("SELECT id FROM designer WHERE mobile='$mobile' LIMIT 1");
	$userMatch = mysql_num_rows($sql); 
    if ($userMatch > 0) {
		echo 'Sorry your mobile number is already registered into the system, <a href="designer_registration.php">click here</a>';
		exit();
	}
	$sql = mysql_query("INSERT INTO designer (login, password, name, mobile, security, securityanswer, email, address, city, state, pin) 
